...
class BarValidator:
    ...
    def validate(self, batch: NormalizedBatch) -> ValidationOutcome:
        ...
        reason = (
            ...
            .when(pl.col("open").is_null() | pl.col("high").is_null() | pl.col("low").is_null())
            .then(pl.lit("ohlc_missing"))
            .when(pl.col("close") <= 0)
            ...
            .when(pl.col("option_type").is_not_null() & pl.col("strike").is_null())
            .then(pl.lit("option_fields_incomplete"))
            .when(pl.col("_dup"))
